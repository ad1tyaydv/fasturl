...
export default function BulkPasswordModal({ isOpen, onClose, selectedBatch, onSuccess }: BulkPasswordProps) {
  ...
  const handleBulkUpdate = async () => {
    setIsLoading(true);
    try {
      const finalPassword = isEditingPassword ? password : selectedBatch?.password;
      const finalExpiry = isEditingExpiry ? expiryDate : selectedBatch?.expiresAt;
  ...
  return (
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 p-4" onClick={() => !isLoading && onClose()}>
      <div className="bg-[#1c1c1c] shadow-2xl w-full max-w-lg p-6 sm:p-10 border border-neutral-800 rounded-xl" onClick={(e) => e.stopPropagation()}>
        <h3 className="text-xl sm:text-2xl font-three mb-8 text-center text-white">Protect Bulk Batch</h3>
        
        <div className="space-y-6 mb-8">
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-xl font-one text-white">Batch Password</label>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              <label className="text-xl font-one text-white">Batch Expiry</label>
            ...
            {!isEditingExpiry && selectedBatch.expiresAt ? (
              <div className="w-full p-3 border border-dashed border-neutral-700 bg-[#1a1a1a] text-neutral-400 font-three rounded-lg">
                Expires on {new Date(selectedBatch.expiresAt).toLocaleDateString()}
              </div>
            ) : (
              <div className="relative group">
                <input 
                  ...
                  className="w-full p-3 border border-neutral-700 bg-[#111111] text-white font-three focus:outline-none focus:border-blue-500 rounded-lg cursor-pointer [color-scheme:dark] pr-12 appearance-none" 
                />
        ...
        <div className="flex justify-end gap-3">
          ...
          >
            {isLoading ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : "Apply to Batch"}
          </Button>
        </div>
      </div>
    </div>
