    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/80 p-4 transition-opacity" onClick={() => !isLoading && onClose()}>
      <div className="bg-[#1c1c1c] shadow-2xl w-full max-w-lg p-6 sm:p-10 border border-neutral-800 rounded-xl" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="space-y-2">
            <div className="flex justify-between items-center">
              ...
              {!isEditingPassword && selectedUrl.password && (
                <button onClick={() => { setIsEditingPassword(true); setPassword(""); }} className="text-blue-500 hover:text-blue-400 cursor-pointer" disabled={isLoading}><IoPencilOutline size={18} /></button>
              )}
            </div>
            {!isEditingPassword && selectedUrl.password ? (
              ...
              <div className="relative flex items-center border border-neutral-700 bg-[#111111] focus-within:border-blue-500 rounded-lg overflow-hidden">
                ...
                <button type="button" onClick={() => setShowPassword(!showPassword)} className="px-4 text-neutral-500 hover:text-white cursor-pointer">{showPassword ? <IoEyeOffOutline size={20} /> : <IoEyeOutline size={20} />}</button>
              </div>
            )}
          </div>
          <div className="space-y-2">
            <div className="flex justify-between items-center"><label className="text-xl font-one text-white">Set Expiry Date</label></div>
        ...
        <div className="flex justify-end gap-3">
          <Button variant="outline" onClick={onClose} disabled={isLoading} className="bg-transparent h-10 px-6 text-white border-neutral-700 hover:bg-[#2a2a2a] cursor-pointer">Cancel</Button>
          <Button onClick={handleUpdate} disabled={isLoading} className="bg-white h-10 text-black hover:bg-gray-200 font-bold min-w-[120px] cursor-pointer">{isLoading ? <Loader2 className="h-5 w-5 animate-spin mx-auto" /> : "Update"}</Button>
        </div>
      </div>
    </div>
  );
