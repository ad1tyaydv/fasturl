...
import { X, LogOut, User, LayoutDashboard, Link2, LayoutGrid, QrCode, BarChart3, Globe, Zap, FileText, Settings, LucideIcon } from "lucide-react";
...
export default function Navbar() {
  ...
  return (
    <>
              ...
              return (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-medium text-sm transition-all duration-200 shrink-0 cursor-pointer ${
                    isActive ? "bg-[#1D9BF0] text-white shadow-sm" : "text-neutral-400 hover:bg-[#222222] hover:text-white"
                  }`}
                >
                  {item.name}
                </button>
                ...
                <NavbarDropDown 
                  ...
                  trigger={
                    <div className="w-9 h-9 rounded-full overflow-hidden bg-[#1e1e1e] border border-white/10 flex items-center justify-center cursor-pointer hover:ring-2 hover:ring-white/20 transition-all">
                      {avatarUrl ? <img src={avatarUrl} alt="Avatar" className="w-full h-full object-cover" /> : <span className="text-white/40 text-xs font-bold">{user?.userName?.[0]?.toUpperCase() ?? "?"}</span>}
                    </div>
                  } 
                />
              </div>
      ...
      <div 
        className={`fixed top-0 right-0 h-full w-full sm:w-[420px] bg-[#141414] z-[101] shadow-2xl transform transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] lg:hidden border-l border-neutral-800 ${
          isSidebarOpen ? "translate-x-0" : "translate-x-full"
        }`}
      >
        <div className="flex flex-col h-full">
          <div className="flex items-center justify-between px-8 py-8 mb-4">
            <h1 className="text-2xl font-three font-bold tracking-tighter text-white">FASTURL</h1>
            ...
            {menuItems.map((item: MenuItem) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-4 px-5 py-4 rounded-xl font-three font-medium text-lg transition-all ${
                    isActive 
