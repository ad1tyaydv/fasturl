...
export default function ResetPassword() {
  ...
  const handleSendOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to send OTP", { id: otpToast });
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    if (otpCode.length !== 6) return toast.error("Please enter all 6 digits of OTP");
    ...
    try {
      ...
      toast.error(error.response?.data?.message || "Failed to reset password", { id: resetToast });
    ...
    <div className="min-h-screen w-full flex flex-col lg:flex-row overflow-hidden bg-background text-foreground selection:bg-[#F07D51] selection:text-white">
      ...
      <div className="hidden lg:flex w-full lg:w-1/2 bg-background p-8 lg:p-20 flex-col relative overflow-hidden lg:min-h-screen border-r border-border/50">
        ...
        >
          ...
          <span className="text-2xl font-bold text-foreground tracking-tight">Fasturl</span>
        </div>
        ...
        <div className="relative z-10 flex-grow flex flex-col justify-center lg:-mt-32">
          ...
          >
            ...
            <p className="text-muted-foreground text-xl lg:text-2xl leading-relaxed max-w-md">
                Don't worry, it happens. Follow the steps to regain access to your account.
            </p>
          </motion.div>
        </div>
      ...
      <div className="w-full lg:w-1/2 p-8 lg:p-20 flex flex-col justify-center bg-background min-h-screen relative">
        ...
        >
          ...
          <span className="text-sm font-bold uppercase tracking-widest">{step === 1 ? "Back to Sign In" : "Go Back"}</span>
        </button>
        ...
        >
          {step === 1 ? (
            <>
              <div className="mb-12">
                <h2 className="text-4xl font-extrabold text-foreground mb-3">Forgot Password</h2>
              ...
              <form onSubmit={handleSendOtp} className="space-y-6">
                <div className="space-y-2">
                  <label className="text-[12px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Email Address</label>
                  <div className="relative">
                    <input
                      ...
                      required
                    />
                    <HugeiconsIcon icon={Mail01Icon} className="absolute right-6 top-1/2 -translate-y-1/2 text-muted-foreground/50 w-6 h-6" />
                  </div>
                </div>
            ...
            <div className="flex flex-col">
              <div className="mb-10 text-center">
                ...
                <h2 className="text-4xl font-extrabold tracking-tight text-foreground mb-4">Verification</h2>
              ...
              <form onSubmit={handleResetPassword} className="space-y-8">
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[12px] font-bold uppercase tracking-widest text-muted-foreground ml-1">New Password</label>
                    <div className="relative">
                      <input
                  ...
                  <div className="space-y-2">
                    <label className="text-[12px] font-bold uppercase tracking-widest text-muted-foreground ml-1">Confirm Password</label>
                    <div className="relative">
                      <input
                      ...
                      <button
                        ...
                        className="absolute right-6 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-[#83c5be] cursor-pointer transition-colors"
                      >
                        {showConfirmPassword ? <HugeiconsIcon icon={ViewOffSlashIcon} size={22} /> : <HugeiconsIcon icon={ViewIcon} size={22} />}
                      </button>
                    </div>
                  </div>
                </div>
